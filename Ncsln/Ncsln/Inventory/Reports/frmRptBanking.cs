...
{
    public partial class frmRptBanking : Form
    {
        CoreClass ObjCore;
        bool firstTime = true;

        public frmRptBanking()
        {
            InitializeComponent();
        ...
        private void FillDGV()
        {
            if (this.cmbBranch.SelectedValue.ToString() == "-1")
            {
                this.dsCashBanking1.Clear();
                this.dsCashBanking1.EnforceConstraints = false;
                this.daCashBanking.SelectCommand.Connection.ConnectionString = this.ObjCore.getConnectionString();
                this.daCashBanking.Fill(this.dsCashBanking1);
            }
            else
            {
                this.dsCashBanking1.Clear();
                this.dsCashBanking1.EnforceConstraints = false;
                this.daCashBankSelected.SelectCommand.Connection.ConnectionString = this.ObjCore.getConnectionString();
                this.daCashBankSelected.SelectCommand.Parameters["@Id"].Value = this.cmbBranch.SelectedValue.ToString();
                this.daCashBankSelected.Fill(this.dsCashBanking1);
            }
        ...
        private void GetTotal()
        {
            try
            {
                ...
                if (this.cmbBranch.SelectedValue.ToString() == "-1")
                {
                    ...
                    {
                        receive += Convert.ToDecimal(this.dgv["amountRec", i].Value);
                        pay += Convert.ToDecimal(this.dgv["paymentAmt", i].Value);
                    }
                } 
                else
                {
        ...
        private void dgv_Paint(object sender, PaintEventArgs e)
        {
            try
            {
                for (int i = 0; i < this.dgv.Rows.Count; i++)
                {
                    ...
                    else if (this.dgv["Id", i].Value.ToString() == "5")
                    {
        ...
        private void dgv_ColumnWidthChanged(object sender, DataGridViewColumnEventArgs e)
        {
            try
            {
                if (!this.firstTime)
                {
                    ...
                    string command = "Update Settings set Value = '" + culumsWidths + "' where Id = 5";
                    this.ObjCore.executeQuery(command, this.ObjCore.getHBCConnectionString());
                }
