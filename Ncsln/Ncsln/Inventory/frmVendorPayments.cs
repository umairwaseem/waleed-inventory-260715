...
{
    public partial class frmVendorPayments : Form
    {
        CoreClass ObjCore;
        private int EditMode = 1;
        private int PaymentId = -1;

        public frmVendorPayments()
        {
            InitializeComponent();
        ...
        {
            this.ObjCore.fillComboBoxOptioni(this.cmbVendors, "Select VendorId, Name From Vendors where VendorId not in (1, 2)", this.ObjCore.getHBCConnectionString());
            this.ObjCore.fillGridComboBoxWithAddNewOption((DataGridViewComboBoxColumn)this.dgv.Columns["VendorId"], "Select VendorId, Name From Vendors", this.ObjCore.getHBCConnectionString());
        ...
        {
            if (!this.ObjCore.CheckRightServer(33, this.PaymentId))
            {
                return;
            }
        ...
        private void SaveUpdate()
        {
            ...
            try
            {
                if (this.EditMode == 1)
                {
                    ...
                    com.CommandText = "spCreateVendorPayment";
                ...
                {
                    com.CommandType = CommandType.StoredProcedure;
                    com.CommandText = "spUpdateVendorPayment";
            ...
            try
            {
                if (e.RowIndex != -1)
                {
                    if (e.ColumnIndex == this.dgv.Columns["Edit"].Index)
                    {
                        ...
                        DataTable dt = this.ObjCore.getDataSet("Select * from VendorPayment Where Id = " + id.ToString()).Tables[0];

                        this.PaymentId = id;
                        this.cmbVendors.SelectedValue = dt.Rows[0]["Vendor_Id"].ToString();
                    ...
                    {
                        if (!this.ObjCore.CheckRightServer(33, this.PaymentId, true))
                        {
                            return;
                        }
                        ...
                        this.ObjCore.executeQuery("Delete from VendorPayment where Id = " + id);
        ...
        private void cmbVendors_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            try
            {
                DataTable dt = this.ObjCore.getDataSet("select dbo.funGetVendorBalance(" + this.cmbVendors.SelectedValue.ToString() + ")", this.ObjCore.getHBCConnectionString()).Tables[0];

                this.txtBalance.Text = dt.Rows[0][0].ToString();
            }
            catch (Exception ex)
            {
