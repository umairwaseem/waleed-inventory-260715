...
{
    public partial class frmSStockOut : Form
    {
        CoreClass ObjCore;
        private int Id;
        public frmSStockOut()
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                if (!this.ObjCore.CheckRightServer(24, this.Id))
                {
                    return;
                }

                if (this.cmbBranch.SelectedValue == null || this.cmbBranch.SelectedValue.ToString() == "-1" )
                {
                    MessageBox.Show("Please select a branch!", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    return;
                }
        ...
        private void PreCheck()
        {
            try
            {
                ...
                if (this.Id == -1)
                {
                    ...
                    {
                        DataTable dt = this.ObjCore.getDataSet("Select dbo.funGetStock(" + this.dgvItems["ItemId", i].Value.ToString() + ")").Tables[0];
                ...
                else
                {
                    for (int i = 0; i < this.dgvItems.Rows.Count - 1; i++)
                    {
                        ...
                        DataTable dtt = this.ObjCore.getDataSet("Select Qty from SStockOutDetail where Item_Id = " + this.dgvItems["ItemId", i].Value.ToString() + " and StockOut_Id = " + this.Id.ToString() + "").Tables[0];
        ...
        private void SaveUpdate()
        {
            ...
            try
            {
                if (this.Id == -1)
                {
                    ...
                    com.CommandText = "spCreateSStockOut";
                    ...
                    com.Parameters.AddWithValue("@Total", this.txtTotal.Text.Trim());
                ...
                {
                    com.CommandType = CommandType.StoredProcedure;
                    com.CommandText = "spUpdateSStockOut";
        ...
        private void dgvShow_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                ...
                if (e.ColumnIndex == this.dgvShow.Columns["Edit"].Index)
                {
                    ...
                    this.dsSStockOutItems1.Clear();
                    this.daSStockOutItem.SelectCommand.Connection.ConnectionString = this.ObjCore.getConnectionString();
                    this.daSStockOutItem.SelectCommand.Parameters["@Id"].Value = this.Id.ToString();
                    this.daSStockOutItem.Fill(this.dsSStockOutItems1);
                ...
                {
                    if (!this.ObjCore.CheckRightServer(24, this.Id, true))
                    {
                        return;
                    }
                    ...
                    {
                        com.CommandType = CommandType.StoredProcedure;
                        com.CommandText = "spDeleteSStockOut";
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                if (this.Id != -1)
                {
                    Reports.frmRptStockOutInvoice obj = new Reports.frmRptStockOutInvoice();
        ...
        private void dgvShow_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                this.dgvShow.Rows[e.RowIndex].Selected = true;
            }
            catch 
            {                
