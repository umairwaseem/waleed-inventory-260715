...
{
    public partial class frmVendorGLiability : Form
    {
        CoreClass ObjCore;
        private int EditMode = 1;
        private int PaymentId = -1;

        public frmVendorGLiability()
        {
            InitializeComponent();
        ...
        {
            if (!this.ObjCore.CheckRightServer(40, this.PaymentId))
            {
                return;
            }
        ...
        private void SaveUpdate()
        {
            ...
            try
            {
                if (this.EditMode == 1)
                {
                    ...
                    com.CommandText = "spCreateVendorGLiability";
                ...
                {
                    com.CommandType = CommandType.StoredProcedure;
                    com.CommandText = "spUpdateVendorGLiability";
        ...
        private void loadDG()
        {
            try
            {
                ...
                this.dsVendorLiability1.Clear();
                this.daVendorLiability.SelectCommand.Connection.ConnectionString = this.ObjCore.getConnectionString();
                //this.daVendorLiability.SelectCommand.Parameters["@id"].Value = this.cmbVendors.SelectedValue.ToString();
                this.daVendorLiability.Fill(this.dsVendorLiability1);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (e.RowIndex != -1)
                {
                    if (e.ColumnIndex == this.dgv.Columns["Edit"].Index)
                    {
                        ...
                        DataTable dt = this.ObjCore.getDataSet("Select * from VendorGLiability Where Id = " + id.ToString()).Tables[0];
                    ...
                    {
                        if (!this.ObjCore.CheckRightServer(40, this.PaymentId, true))
                        {
                            return;
                        }
                        ...
                        this.ObjCore.executeQuery("Delete from VendorGLiability where Id = " + id);
