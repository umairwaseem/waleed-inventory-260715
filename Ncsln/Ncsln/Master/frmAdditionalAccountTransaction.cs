...
{
    public partial class frmAdditionalAccountTransaction : Form
    {
        private const string PermissionName = "Additional Banking";
        private readonly CoreClass objCore;
        private int id = -1;

        public frmAdditionalAccountTransaction()
        {
            InitializeComponent();
            this.objCore = new CoreClass();
        }

        private void frmAdditionalAccountTransaction_Load(object sender, EventArgs e)
        {
        ...
        private void LoadBanks(int selectedBankId)
        {
            try
            {
                DataTable banks = new DataTable();
                using (SqlConnection connection = new SqlConnection(this.objCore.getHBCConnectionString()))
                using (SqlDataAdapter adapter = new SqlDataAdapter("SELECT Id, BankTitle + CASE WHEN ISNULL(AccountNo, '') = '' THEN '' ELSE ' - ' + AccountNo END AS BankDisplay FROM Banks ORDER BY BankTitle", connection))
                {
                    adapter.Fill(banks);
                }
                this.cmbBank.DataSource = banks;
                this.cmbBank.DisplayMember = "BankDisplay";
                this.cmbBank.ValueMember = "Id";
                this.cmbBank.SelectedIndex = -1;
                if (selectedBankId > 0) this.cmbBank.SelectedValue = selectedBankId;
            }
            catch (Exception ex)
            {
        ...
        private void LoadTransactions()
        {
            try
            {
                this.dsAdditionalAccountTransaction1.AdditionalAccountTransaction.Clear();
                using (SqlConnection connection = new SqlConnection(this.objCore.getHBCConnectionString()))
                using (SqlDataAdapter adapter = new SqlDataAdapter("spAdditionalAccountTransactionList", connection))
                {
                    adapter.SelectCommand.CommandType = CommandType.StoredProcedure;
                    adapter.Fill(this.dsAdditionalAccountTransaction1.AdditionalAccountTransaction);
                }
                this.dgv.DataSource = this.dsAdditionalAccountTransaction1.AdditionalAccountTransaction;
        ...
        private int SelectedBankId
        {
            get
            {
                if (this.cmbBank.SelectedValue == null || this.cmbBank.SelectedValue is DataRowView) return -1;
                int value;
                return int.TryParse(this.cmbBank.SelectedValue.ToString(), out value) ? value : -1;
            }
        }

        private void btnSave_Click(object sender, EventArgs e)
        {
            int accountId = this.SelectedAccountId;
            if (accountId <= 0)
            {
            ...
            {
                MessageBox.Show("Please select a bank", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                this.cmbBank.Focus();
                return;
            }
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(this.objCore.getHBCConnectionString()))
                using (SqlCommand command = new SqlCommand(creating ? "spCreateAdditionalAccountTransaction" : "spUpdateAdditionalAccountTransaction", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
                    if (!creating) command.Parameters.Add("@Id", SqlDbType.Int).Value = this.id;
                    command.Parameters.Add("@AdditionalAccountId", SqlDbType.Int).Value = accountId;
                    command.Parameters.Add("@BankId", SqlDbType.Int).Value = this.SelectedBankId;
                    ...
                    command.Parameters.Add("@IsPayment", SqlDbType.Bit).Value = this.rbtnPay.Checked;
                    if (creating)
                    {
        ...
        private void cmbAccount_SelectionChangeCommitted(object sender, EventArgs e)
        {
            ...
            using (frmAdditionalAccount accountForm = new frmAdditionalAccount())
            {
        ...
        private void dgv_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                this.id = selectedId;
                this.cmbAccount.SelectedValue = Convert.ToInt32(row.Cells["colAdditionalAccountId"].Value);
                this.cmbBank.SelectedValue = Convert.ToInt32(row.Cells["colBankId"].Value);
                ...
                bool isPayment = Convert.ToBoolean(row.Cells["colIsPayment"].Value);
                this.rbtnPay.Checked = isPayment;
                this.rbtnReceive.Checked = !isPayment;
        ...
        private void DeleteTransaction(int selectedId)
        {
            ...
            {
                using (SqlConnection connection = new SqlConnection(this.objCore.getHBCConnectionString()))
                using (SqlCommand command = new SqlCommand("spDeleteAdditionalAccountTransaction", connection))
                {
                    command.CommandType = CommandType.StoredProcedure;
        ...
        private void ApplyFilter()
        {
            ...
            this.dsAdditionalAccountTransaction1.AdditionalAccountTransaction.DefaultView.RowFilter = value.Length == 0 ? string.Empty : "AccountName LIKE '%" + value + "%' OR Description LIKE '%" + value + "%' OR BankTitle LIKE '%" + value + "%' OR AccountNo LIKE '%" + value + "%'";
        }
