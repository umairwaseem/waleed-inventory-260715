    ...
    public partial class frmPartner : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("Please enter Partner Name", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    this.txtName.Focus();
                    return;
                }
                ...
                {
                    MessageBox.Show("Rank cannot be zero", "Alert", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
        ...
        private void SaveUpdate()
        {
            ...
            try
            {
                ...
                {
                    com.CommandType = CommandType.StoredProcedure;
                    com.CommandText = "spUpdatePartner";
        ...
        private void LoadDGV()
        {
            try
            {
                this.dsPartner1.Clear();
                this.dsPartner1.EnforceConstraints = false;
                this.daPartner.SelectCommand.Connection.ConnectionString = this.ObjCore.getHBCConnectionString();
                this.daPartner.Fill(this.dsPartner1);
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (e.RowIndex != -1)
                {
                    if (e.ColumnIndex == this.dgv.Columns["Edit"].Index)
                    {
                        ...
                        DataTable dt = this.ObjCore.getDataSet("Select * from Partner where Id = " + SelectedId, this.ObjCore.getHBCConnectionString()).Tables[0];

                        this.txtName.Text = dt.Rows[0]["Name"].ToString();
                        this.txtPhone.Text = dt.Rows[0]["PhoneNo"].ToString();
                        this.txtSalary.Value = Convert.ToDecimal(dt.Rows[0]["Rank"]);
